...
public class KhachHang_DiemTichLuy_View extends JPanel {
    ...
    private JLabel lblTongDiem, lblKHCoDiem, lblKHThuong;
    ...
    public KhachHang_DiemTichLuy_View() {
        ...
        JLabel subtitle = new JLabel("Quản lý điểm tích lũy và thông tin khách hàng");
        ...
        lblTongDiem = createStatLabel("0");
        lblKHCoDiem = createStatLabel("0");
        lblKHThuong = createStatLabel("0");

        statsPanel.add(createStatBox(lblTongDiem, "Tổng điểm", new Color(34, 139, 230)));
        statsPanel.add(createStatBox(lblKHCoDiem, "KH có điểm", new Color(76, 175, 80)));
        statsPanel.add(createStatBox(lblKHThuong, "KH thường", new Color(255, 152, 0)));

        // ===== TABLE =====
        String[] cols = {"Mã KH", "Tên khách hàng", "SĐT", "Email", "Điểm tích lũy", "Hạng", "Trạng thái", "Sửa", "Xóa"};
    ...
    private void loadThongKe() {
        ...
        for (KhachHang kh : dsKH) {
            tongDiem += kh.getDiemTichLuy();
            if (kh.getDiemTichLuy() > 0) khCoDiem++;
            else khThuong++;
        }

        lblTongDiem.setText(String.valueOf(tongDiem));
        lblKHCoDiem.setText(String.valueOf(khCoDiem));
        lblKHThuong.setText(String.valueOf(khThuong));
    }
                ...
                if (type.equals("Sửa")) {
                    SwingUtilities.invokeLater(() -> {
                        ...
                        if (kh != null) {
                            ...
                            loadDiemTichLuyData();  // Reload sau edit
                            loadThongKe();
                        }
                    });
